...
def training(epoch, model, train_pos, test_pos, batch_index, option, optimizer, alpha):
    ...
    for iteration in range(len(batch_index)):
        ...
        if iteration % 20 == 19:
            num = index2
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(epoch, num, total_num,100.* num / total_num, loss.item()))
    ...
    return average_loss, loss.item(), norm_weight
...
def main():
   ...
   #     w.add_graph(model, (dummy_input,))
    ...
    f = open('output/epochInfo.txt', 'w')
    for epoch in range(option.start, option.start + option.nEpochs + 1):
        # training
        avloss,tloss, norm_weight = training(epoch, model, train_pos, test_pos, batch_index, option, optimizer, alpha)
        # validation
        model.cpu()
        ...
        if epoch % 50 is 0:
            ...
            checkpoint(epoch, model)
        if epoch==option.start or epoch==option.start + option.nEpochs: 
            with open('./output/HistVal_TestVel_LabelVel_' + time_loc + 'epoch_' + str(epoch) + '.pkl', 'wb') as ff:
                pickle.dump([res_total, vs_label_pred], ff)
        string_output = "{:.0f} {:.7f} {:.7f} {:.7f} {:.7f} \n".format(epoch, tloss, vloss, vrms,avloss)
